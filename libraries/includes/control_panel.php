    ...
    if (isset($_GET['op']) && $_GET['op'] == 'search') {
        ...
        if ($_admin_) {
            if (G_VERSIONTYPE == 'enterprise') { #cpp#ifdef ENTERPRISE
                ...
                if (!isset($currentUser -> coreAccess['users']) || $currentUser -> coreAccess['users'] != 'hidden') {
                    $controlPanelOptions[]  = array('text' => _USERS, 'font_icon' => 'fa fa-users', 'image' => "32x32/user.png", 'href' => "administrator.php?ctg=users");
                }
            } #cpp#endif
            if (!isset($currentUser -> coreAccess['lessons']) || $currentUser -> coreAccess['lessons'] != 'hidden') {
                $controlPanelOptions[] = array('text' => _LESSONS, 'font_icon' => 'fa fa-sticky-note', 'image' => "32x32/lessons.png", 'href' => "administrator.php?ctg=lessons");
                $controlPanelOptions[] = array('text' => _COURSES, 'font_icon' => 'fa fa-book', 'image' => "32x32/courses.png", 'href' => "administrator.php?ctg=courses");
                $controlPanelOptions[] = array('text' => _DIRECTIONS, 'font_icon' => 'fa fa-th-list', 'image' => "32x32/categories.png", 'href' => "administrator.php?ctg=directions");
            }   
            if (EfrontUser::isOptionVisible('user_types')) {
                $controlPanelOptions[] = array('text' => _ROLES, 'font_icon' => 'fa fa-tags', 'image' => "32x32/user_types.png", 'href' => "administrator.php?ctg=user_types");
            }
            if (EfrontUser::isOptionVisible('groups')) {
                $controlPanelOptions[] = array('text' => _GROUPS, 'font_icon' => 'fa fa-cubes', 'image' => "32x32/users.png", 'href' => "administrator.php?ctg=user_groups");
            }
            if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
            ...
            if (EfrontUser::isOptionVisible('configuration')) {
                $controlPanelOptions[] = array('text' => _CONFIGURATIONVARIABLES, 'font_icon' => 'fa fa-cogs', 'image' => "32x32/tools.png", 'href' => "administrator.php?ctg=system_config");
            }
            if (EfrontUser::isOptionVisible('themes')) {
                $controlPanelOptions[] = array('text' => _THEMES, 'font_icon' => 'fa fa-window-restore', 'image' => "32x32/themes.png", 'href' => "administrator.php?ctg=themes&theme=".$GLOBALS['currentTheme'] -> {$currentTheme -> entity}['id']);
            }
            if (EfrontUser::isOptionVisible('notifications')) {
                $controlPanelOptions[] = array('text' => _EMAILDIGESTS, 'font_icon' => 'fa fa-bullhorn', 'image' => "32x32/notifications.png", 'href' => "administrator.php?ctg=digests");
            }
            if ((!isset($currentUser -> coreAccess['personal_messages']) || $currentUser -> coreAccess['personal_messages'] != 'hidden') && EfrontUser::isOptionVisible('messages')) {
                $controlPanelOptions[] = array('text' => _MESSAGES, 'font_icon' => 'fa fa-envelope-open', 'image' => "32x32/mail.png", 'href' => basename($_SERVER['PHP_SELF'])."?ctg=messages");
            }
            if (EfrontUser::isOptionVisible('online_users')) {
                $controlPanelOptions[] = array('text' => _CONNECTEDUSERS, 'font_icon' => 'fa fa-users', 'image' => "32x32/logout.png", 'href' => "administrator.php?ctg=logout_user");
            }
            if (!isset($currentUser -> coreAccess['users']) || $currentUser -> coreAccess['users'] != 'hidden') {
                $controlPanelOptions[] = array('text' => _EXPORTIMPORTDATA, 'font_icon' => 'fa fa-user', 'image' => "32x32/import_export.png", 'href' => "administrator.php?ctg=import_export");
            }
            if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
            ...
            if (EfrontUser::isOptionVisible('languages')) {
                $controlPanelOptions[] = array('text' => _LANGUAGES, 'font_icon' => 'fa fa-hand-peace', 'image' => "32x32/languages.png", 'href' => "administrator.php?ctg=languages");
            }
            if (EfrontUser::isOptionVisible('statistics')) {
                $controlPanelOptions[] = array('text' => _STATISTICS, 'font_icon' => 'fa fa-file-pdf', 'image' => "32x32/reports.png", 'href' => "administrator.php?ctg=statistics");
            }
            if (EfrontUser::isOptionVisible('backup')) {
                $controlPanelOptions['backup'] = array('text' => _BACKUP." - "._RESTORE, 'font_icon' => 'fa fa-save', 'image' => "32x32/backup_restore.png", 'href' => "administrator.php?ctg=backup");
            }
            if (EfrontUser::isOptionVisible('maintenance')) {
                $controlPanelOptions[] = array('text' => _MAINTENANCE, 'font_icon' => 'fa fa-bolt', 'image' => "32x32/maintenance.png", 'href' => "administrator.php?ctg=maintenance");
            }
            
            if (EfrontUser::isOptionVisible('forum')) {
                $controlPanelOptions[] = array('text' => _FORUM, 'font_icon' => 'fa fa-comments', 'image' => "32x32/forum.png", 'href' => basename($_SERVER['PHP_SELF'])."?ctg=forum");
            }
            if (G_VERSIONTYPE != 'community') { #cpp#ifndef COMMUNITY
                if (G_VERSIONTYPE != 'standard') { #cpp#ifndef STANDARD
